/* ============================================================
   ...
   ничего менять не надо: имена целей от страны не зависят.
     ...
     cta_click      клик по главной кнопке (index.html — проверка)
     payment_click  клик по кнопке оплаты (index.html — проверка)
     form_fallback  клик по ссылке «форма не открылась» (диагностика)

     Второй оффер живёт на new.html — разработка паспорта с нуля.
     У него свои цели, иначе два спроса сольются в один и замер
     ничего не покажет:

     cta_click_new      клик по главной кнопке (new.html)
     payment_click_new  клик по кнопке оплаты (new.html)
     form_fallback_new  «форма не открылась» на new.html

     lead_submit    заявка отправлена — ОДНА на обе страницы, её шлёт
                    thanks.html. Какой это был оффер, видно двумя
                    способами: в Метрике — по странице входа и по
                    метке кампании, в самой заявке — по ответу на
                    обязательный вопрос «Что нужно» (проверить
                    существующий / разработать новый / консультация).
